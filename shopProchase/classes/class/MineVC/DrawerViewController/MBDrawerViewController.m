...
@interface MBDrawerViewController ()<UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) NSHashTable<id<MBDrawerViewControllerDelegate>> *delegates;

@end

@implementation MBDrawerViewController
...
- (void)bind:(id<MBDrawerViewControllerDelegate>)delegate {
    if (!delegate) return;
    if (![delegate conformsToProtocol:@protocol(MBDrawerViewControllerDelegate)]) return;
...
- (void)sendMenuDidAppearNotification {
    for (id<MBDrawerViewControllerDelegate> delegate in [self.delegates setRepresentation]) {
...
- (NSHashTable<id<MBDrawerViewControllerDelegate>> *)delegates {
    if (!_delegates) {
